...
tenkey = {"<96>":"0", "<97>":"1", "<98>":"2", "<99>":"3", "<100>":"4", "<101>":"5", "<102>":"6", "<103>":"7", "<104>":"8", "<105>":"9"}
    ...
    if current_window_name == WINDOW_NAME or current_window_name == ZOOM_WINDOW_NAME:
        ...
        # west+1
            ...
            if press_shift == 0:
                action = "WEST+1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(-1, 0, 0, 0, current_img.width, current_img.height)
            else:
                action = "WEST-1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(1, 0, 0, 0, current_img.width, current_img.height) 
        ...
        # north+1
            ...
            if press_shift == 0:
                action = "NORTH+1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(0, -1, 0, 0, current_img.width, current_img.height)
            else:
                action = "NORTH-1"
                if box_on_mouse:
                   box_on_mouse.resize_delta(0, 1, 0, 0, current_img.width, current_img.height)
        ...
        # east+1
            ...
            if press_shift == 0:
                action = "EAST+1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(0, 0, 1, 0, current_img.width, current_img.height)
            else:
                action = "EAST-1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(0, 0, -1, 0, current_img.width, current_img.height)
        ...
        # south+1
            ...
            if press_shift == 0:
                action = "SOUTH+1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(0, 0, 0, 1, current_img.width, current_img.height)
            else:
                action = "SOUTH-1"
                if box_on_mouse:
                    box_on_mouse.resize_delta(0, 0, 0, -1, current_img.width, current_img.height)
...
def on_mouse(event, x, y, flags, param):
    ...
    # 앵커 잡고 드래그 : 박스 크기 조절
        ...
        if box_on_mouse and anchorbox.find_mouse_overed_anchor(mouse_x, mouse_y):
            anchor_name, anchor_center = anchorbox.find_mouse_overed_anchor(mouse_x, mouse_y)
            ...
            box_on_mouse.resize(xmin, ymin, xmax, ymax, current_img.width, current_img.height)
...
parser.add_argument('--class_list', default='./data/argos_v6.2.txt', type=str, help='Class label (txt file)')
...
if __name__ == '__main__':
    ...
    notice_text = Text(classes=named_class, text="Selected label : {}".format(Util.get_class_label(0, named_class)))
    ...
    while True:
        ...
        cv2.line(current_img.img, (mouse_x, 0), (mouse_x, current_img.height), color, thickness)
        cv2.line(current_img.img, (0, mouse_y), (current_img.width, mouse_y), color, thickness)
        cv2.rectangle(current_img.img, (prev_x, prev_y), (mouse_x, mouse_y), color, thickness)
        ...
        if action:
            ...
            if action.isdigit():
                ...
                notice_text.set_text("Selected label : {}".format(Util.get_class_label(current_label, named_class)))
